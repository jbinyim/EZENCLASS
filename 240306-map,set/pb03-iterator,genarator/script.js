...
// itrerble : 순차적인, 순서대로 무언가를 처리할 수 있는
// 순서대로 무언가에 연산작업을 처리할수 있는 객체
// 순서 => index값처럼 순번 할당 값
// 일반객체 => 이터러블 객체
// 문자열, 배열, map, set in 자바스크립트

// 태생이 이터러블 객체가 아닌 요소들은 반복문을 사용할 수 있는 기회가 완전 박탈인가?
...
// 배열, 이터러블한 객체가 되기 위해서 클래스 생성자 함수에는 심볼이 존재하는데 "Symbol.iterartor" => 심볼 함수가 존재하기 때문에 배열은 이터러블한 객체가 될 수 있음

// 이터러블한 객체가 되기 위해서는 해당 객체에 반드시 이터레이터 함수를 갖고있어야한다
