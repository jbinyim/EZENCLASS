...
// 1) js객체 => 일반객체 => 배열객체
// 2) 일반객체
// 장점 : key, value 값을 가지고 있는 프로퍼티 데이터 체겨적으로 보관,관리,사용
// 단점 : for...in => 객체 안에 있는 value값만을 찾아와서 직접적으로 반복문을 돌릴 수 있는 방법X
// 객체안에 구성되었는 아이템들은 배열처럼 인덱스 번호를 할당받지 못한다

// 3) 배열객체
// 장점 : 배열안에 포함되어있는 아이템들이 고유의 인덱스값을 가지고 있으며, for문을 활용한 반복문으로 업무의 효율성 극대화
// 단점 : 객체 처럼 key와 value 형태로 구성된 체계적인 데이터 관리X

// solution : 일반객체 & 배열객체의 장점만 모아서 하나로 만들어 주면 어떨까 => Map

// key 와 value 값으로 구성된 아이템을 가질 수 있다
// 각 아이템별 인덱스 값을 할당
// for의 반복문을 활용해서 언제든지 값을 추출해 올 수 있다
...
// myCup.set("type", "mini").set("purpos", "daily");
// console.log(myCup);
...
// 동일한 데이터 하나의 객체안에 들어오면 안되는 상황

// 일반객체 + 배열객체 + Map + 데이터 중복 => Set()
