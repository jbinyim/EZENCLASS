...
// 함수를 구현하려면 어떻게 해야할까?
// 함수를 구현하는 방법 3가지 (ver.2024)
// 1. 클래스 함수를 구현하는 방법
// 2. 익명의 함수를 구현하는 방법
// 3. 화살표 함수를 구현하는 방법(가장 최신이고 많이 사용하지만 항상은 아니다)
...
// const num = Number(prompt("몇 까지 숫자를 더할까요?"));
// function calcSum(n) {
//   let sum = 0;
//   for (let i = 1; i <= n; i++) {
//     sum += i;
...
// JS 권장사항!
// 1. var 보다는 let 과 const 키워드로 변수를 선언해라
...
// 2. 가급적이면 전역 스코프의 변수설언은 피해라
// 3. 그래서 var, let 보다는 const에 적응해라

// 즉시실행 함수 (고차함수에서 많이 사용함)
...
// 화살표 함수에서는 매개변수가 존재하지 않는 경우에는 실행문을 보호하는 중괄호와 & 값을 반한 시켜주는 return문을 생략해도 문제없이 작동가능!!
// let hi = () => `안녕하세요`;
...
// 3. 함수의 실행문에서 굳이 중괄호 & renturn문을 사용하지 않아도 구현할 수 있또록 해주기위한 목적
...
// 모든 프로그래밍 언어에서 1급시민이ㄷ 되려면 다음과 같은 조건 부합

// 1) 함수를 변수에 값으로 할당할 수 있어야 한다
// 2) 함수가 또 다른 함수의 매개변수 혹은 인자값으로 사용될 수 있어야 한다.
// 3) 함수가 또 다른 함수의 return 안에 반환값으로 들어올 수 있어야 한다. => 고차함수
...
// clearInterval() :  특정 조건에 따라서 반복싱행 함수를 멈추게 하는 함수
// 단락회로평가
...
// setTimeout : 특정시간이 경화한 이후에 작동하게 만드는 함수
// heap / callstack / Que
